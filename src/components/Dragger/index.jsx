...
export default class Dragger extends PureComponent {
    ...
    onDragStart(event) {
        ...
        if (this.props.hasDraggerHandle) {
            if (event.target.className !== 'handle') return false;
        }

        if(this.props.hasCancelHandle){
            if (event.target.className === 'cancel') return false;
        }

        if (this.props.static === true) return false;
    ...
    render() {
        ...
        return (
            <div className={`${fixedClassName}WrapDragger`}
                 style={{  touchAction: 'none!important', transform: `translate(${x}px,${y}px)`, ...style, }}
